...
bool indirect_reservoir_update(
    ...
    if (visible && ph_rand_next_float(frag_rnd_state) < required_rng) {
...
bool indirect_reservoir_merge(
    ...
    float source_weight = source_sample_weight * source.weight * source.total_samples * jacobian;
    if (source_weight < MAX_SOURCE_WEIGHT && indirect_reservoir_update(reservoir, source.smple, source_weight, source.total_samples, visible, rng)) {
...
void indirect_reservoir_reuse(
    ...
    if (source_weight >= MAX_SOURCE_WEIGHT || !indirect_reservoir_update(reservoir, source.smple, source_weight, source.total_samples, true, ignored)) return;
...
bool indirect_reservoir_validate_visiblity(inout IndirectReservoir reservoir, vec3 rt_pos) {
    ...
    if (!ray_result_is_hit(result)) {
        ...
        if (dot(pos_diff, pos_diff) < 0.05f) return true;
    }
...
vec3 indirect_reservoir_get_final_color(inout IndirectReservoir reservoir) {
    return clamp(indirect_sample_get_color(reservoir.smple) * reservoir.weight, 0.0f, 65504.0);
}
