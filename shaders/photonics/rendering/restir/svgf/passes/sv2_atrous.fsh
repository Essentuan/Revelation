...
void main() {
    ...
    vec2 depth_gradient = vec2(dFdx(D0), dFdy(D0));
    ...
    if (pass_weight > 0.0f) {
        ...
        for (int i = 0; i < 9; ++i) {
            ivec2 sample_offset = step_width * offset[i];
            ivec2 p = texel + sample_offset;
            ...
            fetch_svgf_data(p, Di, Ni, DSi, GSi, sample_data);
                ...
                atrous_state_filter_neighbor(i, sample_offset, di_state, depth_gradient, D0, Di, N0, Ni, DS0, DSi, sample_data.x, color_data.xy, frame_count);
            #endif

            #if defined PH_ENABLE_GI && defined PH_RESTIR_COMBINED_GI
                atrous_state_filter_neighbor(i, sample_offset, gi_state, depth_gradient, D0, Di, N0, Ni, GS0, GSi, sample_data.y, color_data.zw, frame_count);
            #endif
        }
