...
bool direct_reservoir_merge(
    ...
    if (source_weight < MAX_SOURCE_WEIGHT && direct_reservoir_update(reservoir, source.smple, source_weight, source.total_samples, rng)) {
...
void direct_reservoir_reuse(
    ...
    if (source_weight >= MAX_SOURCE_WEIGHT || !direct_reservoir_update(reservoir, source.smple, source_weight, source.total_samples, ignored)) return;
...
vec3 direct_reservoir_get_final_color(inout DirectReservoir reservoir) {
    return clamp(direct_sample_get_color(reservoir.smple) * reservoir.weight, 0.0f, 65504.0);
}
